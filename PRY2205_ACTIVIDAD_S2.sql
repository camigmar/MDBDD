...
SELECT
    ...
    TO_CHAR(f.fecha, 'DD "de" Month YYYY') AS "Fecha Emisión",

    -- Los rut siempre deberán tener un largo de 10 colocando 0 a la izquierda los que no cumplan con el largo establecido:
    LPAD(f.rutcliente, 10, '0') AS "RUT Cliente",

    '$' || TO_CHAR(ROUND(f.neto),  '999G999G999') AS "Monto Neto",
    '$' || TO_CHAR(ROUND(f.iva),   '999G999G999') AS "Monto IVA",
    '$' || TO_CHAR(ROUND(f.total), '999G999G999') AS "Total Factura",

    -- Para clasificar el total la empresa ha decidido una escala la cual será de 0 a 50000 se considera 
    -- ”Bajo”, de 50001 hasta 100000 se considera “Medio” y cualquier monto
    -- superior a 100000 se considera “Alto”:
    CASE 
    ...
    -- la clasificación de formas de pago cuando el código de pago sea 1
    -- entonces 'EFECTIVO', cuando sea 2 entonces 'TARJETA DEBITO', cuando sea 3
    -- entonces 'TARJETA CREDITO' y cualquier otro será 'CHEQUE':
    CASE f.codpago
...
-- Considerar SOLO facturas emitidas el año anterior al año de ejecución:
WHERE EXTRACT(YEAR FROM f.fecha) = EXTRACT(YEAR FROM SYSDATE) - 1

-- Como medida de optimización de fin de año se requiere que siempre se muestre las
-- fechas en orden descendente y por monto neto de forma descendente
ORDER BY f.fecha DESC, f.neto DESC;

-- Caso 2: Clasificación de Clientes

SELECT
    -- El RUT de los clientes debe mostrarse ordenado de derecha a izquierda y rellenando
    -- los espacios en blanco con *
     LPAD(REVERSE(rutcliente), 12, '*') AS "RUT",
    
     nombre AS "Cliente",
    
    -- Manejar los datos nulos en los campos teléfono (Sin teléfono), comuna (Sin comuna)
    ...
    -- Es necesario categorizar el crédito en base a la relación SALDO / CREDITO y definir:
    -- Bueno: Si es menor al 50%. Mostrar diferencia entre crédito y saldo formateado.
    -- Regular: Entre 50% y 80%. Mostrar saldo actual formateado.
    -- Crítico: Mayor al 80%
    CASE
        WHEN (saldo / credito) < 0.5 THEN
            'Bueno ($' || TO_CHAR(ROUND(credito - saldo, 0), '999G999G999') || ')'
        WHEN (saldo / credito) BETWEEN 0.5 AND 0.8 THEN
            'Regular ($' || TO_CHAR(ROUND(saldo, 0), '999G999G999') || ')'
        ELSE
            'Crítico'
    END AS "Estado Crédito",
    
    --y correo (Correo no registrado) y Para priorizar el envío de información, se requiere obtener el dominio de los correos:
    CASE
...
FROM cliente
-- Recuperar los registros de los clientes cuyo estado sea A con crédito mayor a 0
WHERE estado = 'A'
  AND credito > 0
ORDER BY nombre ASC;

-- Caso 3: Stock de productos
-- !!!! Usar variables de sustitución para tipo de cambio dólares, umbrales de stock bajo y alto
SELECT
    codproducto AS "ID",
    
    descripcion AS "Descripción de Producto",
    
    -- • Mostrar el valor de compra en dólares. Aquellos valores nulos mostrar ‘Sin registro’
    -- • Realizar la conversión del valor de compra en dólares en pesos, considerando el tipo
    -- de cambio en pesos CLP
    NVL(TO_CHAR(valorcompradolar, '999G999D99') || ' USD', 'Sin registro') AS "Compra en USD",
    
    CASE
        WHEN valorcompradolar IS NULL THEN 'Sin registro'
        ELSE '$' || TO_CHAR(ROUND(valorcompradolar * &TIPOCAMBIO_DOLAR, 0), '999G999G999') || ' PESOS'
    END AS "USD convertido",
    
  
    NVL(TO_CHAR(totalstock), 'N/A') AS "Stock",
    
    -- Genera ALERTAS DE STOCK de 4 niveles:
    -- 1. Sin datos: Si TOTALSTOCK es nulo.
    -- 2. ¡ALERTA stock muy bajo!: Si el stock está por debajo de UMBRAL_BAJO.
    -- 3. ¡Reabastecer pronto!: Si el stock está entre UMBRAL_BAJO y UMBRAL_ALTO.
    -- 4. OK: Si supera UMBRAL_ALTO
    CASE
    ...
    -- Los descuentos de cada producto serán aplicados solo a aquellos que tengan sobre
    -- 80 productos de sí mismo (totalstock) y este será equivalente al 10% de su valor unitario.
    CASE
        WHEN totalstock > 80 THEN
            '$' || TO_CHAR(ROUND(vunitario * 0.9, 0), '999G999G999')
        ELSE 'N/A'
    END AS "Precio Oferta"
    
FROM producto
-- Se mostrarán SOLO los productos que tenga en la descripción zapato y cuya procedencia sea i
WHERE 
