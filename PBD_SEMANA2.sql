...
ALTER TABLE USUARIO_CLAVE MODIFY nombre_usuario VARCHAR2(50);
ALTER TABLE USUARIO_CLAVE MODIFY clave_usuario VARCHAR2(50);

VAR v_fecha_proceso VARCHAR2(20);

BEGIN
    :v_fecha_proceso := TO_CHAR(SYSDATE, 'DD/MM/YYYY');
END;
/
...
DECLARE
    ...
    v_tres_letras_nom   VARCHAR2(3);                      
    v_largo_nombre      NUMBER;                           
    v_ultimo_digito_sal NUMBER;                           
    v_anios_trabajados  NUMBER;                           
    v_tercer_digito_run NUMBER;                           
    v_ultimos_tres_sal  NUMBER;                           
    v_dos_letras_apell  VARCHAR2(2);                      
    v_mes_anio          VARCHAR2(6);                      
    v_nombre_usuario    VARCHAR2(50);                     
    v_clave_usuario     VARCHAR2(50);                     
    v_nombre_completo   VARCHAR2(100);                   
    v_contador          NUMBER := 0;                      
    v_total_empleados   NUMBER := 23;
    BEGIN
    
    v_fecha_date := TO_DATE(:v_fecha_proceso, 'DD/MM/YYYY');
    ...
    FOR i IN 100..320 LOOP
        ...
        BEGIN
            ...
            v_anio_nacimiento := EXTRACT(YEAR FROM v_fecha_nac);

            v_anios_trabajados := ROUND(MONTHS_BETWEEN(v_fecha_date, v_fecha_contrato) / 12);
            
            IF v_id_estado_civil = 10 THEN
                v_letra_est_civil := 'c';
            ELSIF v_id_estado_civil = 20 THEN
                v_letra_est_civil := 'd';
            ELSIF v_id_estado_civil = 30 THEN
                v_letra_est_civil := 's';
            ELSIF v_id_estado_civil = 40 THEN
                v_letra_est_civil := 'v';
            ELSIF v_id_estado_civil = 50 THEN
                v_letra_est_civil := 'e';
            ELSIF v_id_estado_civil = 60 THEN
                v_letra_est_civil := 'u';
            ELSE
                v_letra_est_civil := 'x';  
            END IF;
            
            v_tres_letras_nom := SUBSTR(v_pnombre_emp, 1, 3);
            
            v_largo_nombre := LENGTH(v_pnombre_emp);
            
            v_ultimo_digito_sal := MOD(ROUND(v_sueldo_base), 10);
            
            v_nombre_usuario := v_letra_est_civil || 
                               v_tres_letras_nom || 
                               v_largo_nombre || 
                               '*' || 
                               v_ultimo_digito_sal || 
            ...
            v_ultimos_tres_sal := MOD(ROUND(v_sueldo_base) - 1, 1000);
            
            IF v_id_estado_civil = 10 OR v_id_estado_civil = 60 THEN
                v_dos_letras_apell := LOWER(SUBSTR(v_appaterno_emp, 1, 2));
                
            ELSIF v_id_estado_civil = 20 OR v_id_estado_civil = 30 THEN
                v_dos_letras_apell := LOWER(SUBSTR(v_appaterno_emp, 1, 1) || SUBSTR(v_appaterno_emp, -1, 1));
                
            ELSIF v_id_estado_civil = 40 THEN
                v_dos_letras_apell := LOWER(SUBSTR(v_appaterno_emp, -3, 1) || SUBSTR(v_appaterno_emp, -2, 1));
                
            ELSIF v_id_estado_civil = 50 THEN
                v_dos_letras_apell := LOWER(SUBSTR(v_appaterno_emp, -2, 2));
                
            ELSE -- algún estado civil q no esté, usamos las 2 primeras letras
                v_dos_letras_apell := LOWER(SUBSTR(v_appaterno_emp, 1, 2));
            END IF;
            
            v_mes_anio := TO_CHAR(v_fecha_date, 'MMYYYY');
            
            v_clave_usuario := v_tercer_digito_run || 
                              (v_anio_nacimiento + 2) || 
                              LPAD(v_ultimos_tres_sal, 3, '0') ||
                              v_dos_letras_apell || 
                              v_id_emp || 
                              v_mes_anio;
            
            v_nombre_completo := v_pnombre_emp || ' ' || NVL(v_snombre_emp || ' ', '') || v_appaterno_emp || ' ' || v_apmaterno_emp;
        ...
        EXCEPTION
            -- si el empleado no existe, continuamos con el siguiente
    ...
    ELSE
        -- si no se procesaron todos los empleados, se revierten los cambios
        ROLLBACK;
        DBMS_OUTPUT.PUT_LINE('ERROR: No se procesaron todos los empleados.');
        DBMS_OUTPUT.PUT_LINE('Esperados: ' || v_total_empleados || ' - Procesados: ' || v_contador);
    END IF;
    
EXCEPTION
    WHEN OTHERS THEN
        -- cualquier error se revierten cambios
        ROLLBACK;
